...
function ScheduleView() {
    ...
    const [employees, setEmployees] = useState<Record<number, { id: number; name: string; minHours: number }>>({});
    ...
    React.useEffect(() => {
        const fetchEmployees = async () => {
            try {
                ...
                const transformed: Record<number, { id: number; name: string; minHours: number }> = {};
                Object.values(data).forEach((item: any) => {
                    transformed[item.id] = { id: item.id, name: item.employeeName, minHours: item.minHours };
                });
